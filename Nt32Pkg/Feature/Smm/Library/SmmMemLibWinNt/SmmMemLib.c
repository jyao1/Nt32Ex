...
UINTN                 mUefiMemoryMapEntryCount;
  ...
  )
{
  ...
  if (mSmmReadyToLock) {
    ...
    if (!InValidCommunicationRegion) {
      //
      // For NT32, add additional check that this memory region is NOT in UEFI memory map,
      // because NT32 may load RT driver to DLL memory, which is NOT RuntimeCode.
      //
      InValidCommunicationRegion = TRUE;
      MemoryMap = mUefiMemoryMap;
      for (Index = 0; Index < mUefiMemoryMapEntryCount; Index++) {
        if ((Buffer >= MemoryMap->PhysicalStart) &&
          (Buffer + Length <= MemoryMap->PhysicalStart + LShiftU64(MemoryMap->NumberOfPages, EFI_PAGE_SHIFT))) {
          InValidCommunicationRegion = FALSE;
        }
...
//    DEBUG((EFI_D_ERROR, "  0x%016lx - 0x%016lx\n", MemoryMap->PhysicalStart, MemoryMap->PhysicalStart + LShiftU64(MemoryMap->NumberOfPages, EFI_PAGE_SHIFT)));
  ...
  //
  // Reserve all UEFI memory map for SMRAM
  //
  mUefiMemoryMapEntryCount = MemoryMapEntryCount;
  mUefiMemoryMap = AllocateCopyPool (mUefiMemoryMapEntryCount*DescriptorSize, MemoryMap);
