  ...
  )
{
  ...
  ASSERT(HobList != NULL);
  GuidHob = GetNextGuidHob(&gEfiWinNtThunkProtocolGuid, HobList);
  ASSERT (GuidHob != NULL);
  ...
  )
{
  ...
  //
  // Send the print string to Console Output device
  //
  DebugMessageWrite ((UINT8 *)Buffer, (UINT32)AsciiStrLen (Buffer));
}
